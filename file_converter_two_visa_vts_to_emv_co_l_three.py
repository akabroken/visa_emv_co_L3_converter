...
def parse_inovant_log(log_data):
    messages = []
    print(f"Parsing log data with length {len(log_data)} characters...")
    print("Log data preview (first 500 characters):")
    print(log_data[:5000])
    print("Starting message parsing...")
    print("========================================")
    print("Log data split by ISO^ blocks...")
    print("========================================")
    print("Note: This may take a while for large logs...")
    print("========================================")
    print("Processing log data...")
   
    # Split log by ISO^ blocks instead of 'Case'
    # Split on any position before 'ISO^' (to keep ISO^ at start of each message)
    raw_messages = re.split(r'(?=ISO\^)', log_data.strip())
    print(f"Raw messages split into {len(raw_messages)} blocks. Processing each block...")
    print(f"Raw messages preview (first 3000 blocks):")
    print("\n".join(raw_messages[:3000]))
    print("========================================")

    # Remove empty or whitespace-only entries
    raw_messages = [msg.strip() for msg in raw_messages if msg.strip()]
    print(f"Found {len(raw_messages)} raw message blocks.")
    print(f"{raw_messages}")
    for msg_block in raw_messages:
        lines = msg_block.split('\n')
        header_line = lines[0].strip()
        rest = '\n'.join(lines[1:])
        print(f"Processing message block rest: {rest}")
        print(f"Processing message block header_line: {header_line}")

        # Extract MTI more flexibly
        mti_match = re.search(r'\b(01[01]0)\b', header_line)
        if mti_match:
            mti = mti_match.group(1)
        else:
            mti_field = re.search(r'sID:MTI.*?sDATA:(01[01]0)', rest)
            if mti_field:
                mti = mti_field.group(1)
                print(f"⚠️ Fallback MTI found: {mti}")
            else:
                continue  # skip if no MTI

        class_type = "Request" if mti == "0100" else "Response"
        source, destination = ("N/A", "TestIssuer|Network") if class_type == "Request" else (
            "TestIssuer|Network", "N/A")

        # Field extraction pattern (skip Hxx fields)
        #field_pattern = r'~sID:([^ H][^ ]*)\s+sNAME:([^~]+?)\s+sDATA:([^~\s]+)(?:\s+sACDATA:([^~\s]+))?'
        # Field extraction pattern with better handling of edge cases
        #field_pattern = r'~sID:([^ H][^ ]*)\s+sNAME:([^~]*?)\s+sDATA:(\{[^~]*?\}|[^\s~]+?)(?:\s+sACDATA:(\{[^~]*?\}|[^\s~]+))?'
        #field_pattern = r'~sID:([^ H][^ ]*)\s+sNAME:([^~]*?)\s+sDATA:(\{[^~]*?\}|[^\s~]+?)(?:\s+sACDATA:(\{[^~]*?\}|[^\s~]+))?(?=\s+~|$)'
        #field_pattern = r'~sID:([^ H][^ ]*)\s+sNAME:([^~]*?)\s+sDATA:(\{[^~]*?\}|[^\s~]+?)(?:\s+sACDATA:(\{[^~]*?\}|[^\s~]+))?(?=\s+~|$)'

         # Improved regex pattern
        field_pattern = r'~sID:([^ H][^ ]*)\s+sNAME:([^~]*?)\s+sDATA:(\{[^~]*?\}|[^\s~]+?)(?:\s+sACDATA:(\{[^~]*?\}|[^\s~]+))?(?=\s+~|$)'
        
        print(f"Using field pattern: {field_pattern}")
        print("Extracting fields from message...")
        print(f"Using field pattern: {header_line}")
        print("========================================")
        all_fields = re.findall(field_pattern, header_line, re.DOTALL)

        # Debugging: print matched fields
        print(f"Matched fields ({len(all_fields)}):")
        for fid, fname, fdata, facdata in all_fields:
            print(f" - {fid}: '{fdata}' | ACData: '{facdata}'")

        incoming_fields = []
        outgoing_fields = []

        # Always assign fields based on class_type, not text matching
        if class_type == "Request":
            incoming_fields = [{
                'id': fid.strip(),
                'name': fname.strip(),
                'value': fdata.strip(),
                'acdata': acdata.strip() if acdata else None
            } for fid, fname, fdata, acdata in all_fields]
        else:
            outgoing_fields = [{
                'id': fid.strip(),
                'name': fname.strip(),
        ...
        print(f"Parsed message: MTI={mti}, Class={class_type}")
        print(f"Incoming Fields Count: {len(incoming_fields)}")
        print(f"Outgoing Fields Count: {len(outgoing_fields)}")
